  ...
  return (
    <div className="onb-done-screen">
      ...
      <div style={{ background: "#F9F8FF", borderRadius: 16, padding: "16px", width: "100%", marginBottom: 20, border: "1px solid #E8E5FF" }}>
        ...
        {[
          { label: "Account Created", status: "Done", tone: "done" },
          {
            label: "KYC Documents",
            status: kycVerified ? "Verified" : kycSubmitted ? "Submitted" : "Pending",
            tone: kycVerified ? "done" : kycSubmitted ? "submitted" : "pending",
          },
          { label: "Bank Account", status: bankConnected ? "Added" : "Pending", tone: bankConnected ? "done" : "pending" },
          {
            label: "Membership Active",
            status: membershipActive ? "Active" : membershipSubmitted ? "Submitted" : "Not submitted",
            tone: membershipActive ? "done" : membershipSubmitted ? "submitted" : "pending",
          },
        ].map((item) => (
          <div key={item.label} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px solid #F3F1FF" }}>
            <div style={{ color: item.tone === "done" ? "#16A34A" : item.tone === "submitted" ? "#D97706" : "#9CA3AF", display: "flex" }}>
              {item.tone === "done" ? <BadgeCheck size={16} /> : <Crown size={16} />}
            </div>
            <span style={{ flex: 1, fontSize: 13, color: item.tone === "done" ? "#0F0A3C" : "#6B7280", fontWeight: item.tone === "done" ? 600 : 400 }}>
              {item.label}
            </span>
            <span style={{ fontSize: 11, fontWeight: 700, color: item.tone === "done" ? "#16A34A" : item.tone === "submitted" ? "#D97706" : "#D97706" }}>
              {item.status}
            </span>
          </div>
        ))}
